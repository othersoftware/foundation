...
import path__default, { basename, parse, resolve } from "node:path";
function scan(dir, extensions = [".vue"], list = []) {
  ...
  files.forEach((file) => {
    const resolved = path__default.join(dir, file);
...
function collect(config, sources, namespace = void 0) {
  ...
  Object.entries(sources).forEach(([local, vendor]) => {
    ...
    scanComponents(resolve(config.root, local), components, void 0, namespace);
  });
...
function writeComponentsDeclarations$1(config, options, target, components) {
  let directory = path__default.resolve(config.root, target);
  let declarations = path__default.resolve(directory, "components.d.ts");
  let lines = [];
  ...
  lines.push(``);
  components.forEach((resolved) => lines.push(`  export { default as ${resolved.global} } from '${normalizePath(path__default.relative(directory, resolved.path))}';`));
  lines.push(`}`);
  lines.push(``);
...
function writeVueDeclarations(config, target, components) {
  let directory = path__default.resolve(config.root, target);
  let declarations = path__default.resolve(directory, "vue.d.ts");
  let lines = [];
  lines.push(`// THIS FILE IS AUTOGENERATED!`);
  lines.push(`// DO NOT EDIT!`);
  lines.push(``);
  ...
  components.forEach((resolved) => lines.push(`    ${resolved.global}: typeof import('${normalizePath(path__default.relative(directory, resolved.path))}')['default'],`));
  lines.push(`  }`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`export {}`);
...
function writeComponentsDeclarations(config, options, target) {
  let directory = path__default.resolve(config.root, target);
  let declarations = path__default.resolve(directory, "views.d.ts");
  let lines = [];
  lines.push(`// THIS FILE IS AUTOGENERATED!`);
  lines.push(`// DO NOT EDIT!`);
  lines.push(``);
...
function writePhpstormMeta(config, target, views) {
  let directory = path__default.resolve(config.root, target);
  let meta = path__default.resolve(directory, ".phpstorm.meta.php");
  let lines = [];
  let compiled = [];
...
export {
  autoloader as default
};
//# sourceMappingURL=other-software-autoloader.js.map
